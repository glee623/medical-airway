...
def get_df_stone(k_fold, data_dir, data_folder, out_dim = 1, out_dim2=0, use_meta = False, use_ext = False, side_task=0):
    '''
    ...
    df_train = pd.read_csv(os.path.join(data_dir, data_folder, 'train_half.csv'))
    ...
    df_train['filepath'] = df_train['file_name'].apply(lambda x: os.path.join(data_dir, f'{data_folder}train', x))  # f'{x}.jpg'
    ...
    patients_uniq = df_train['patient_id'].unique()
    print(f'Original dataset의 사람 인원수 : {len(patients_uniq)}')
    ...
    filename2fold = {num: i % k_fold for i, num in enumerate(patients_uniq)}
    ...
    # test data
    df_test = pd.read_csv(os.path.join(data_dir, data_folder, 'train.csv'))
    df_test['filepath'] = df_test['file_name'].apply(lambda x: os.path.join(data_dir, f'{data_folder}test', x)) # f'{x}.jpg'
    ...
    if side_task == 1:
        diagnosis2idx2 = {d: idx for idx, d in enumerate(sorted(df_train.alphabet.unique()))}
        df_train['target2'] = df_train['alphabet'].map(diagnosis2idx2)
...
class MMC_ClassificationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_meta:
            ...
            patient_id = torch.tensor(row['patient_id'])
        ...
        if self.mode == 'valid':
            if self.side_task:
                return data, torch.tensor(self.csv.iloc[index].target).long(), torch.tensor(self.csv.iloc[index].target2).long(), patient_id, self.csv.iloc[index].file_name
            else:
                return data, torch.tensor(self.csv.iloc[index].target).long(), patient_id, self.csv.iloc[index].file_name
        else:
            if self.side_task:
...
def get_transforms(image_size):
    ...
    transforms_train = albumentations.Compose([
        ...
        albumentations.CLAHE(clip_limit=5.0, p=0.7),
