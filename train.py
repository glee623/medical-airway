...
from models import Effnet_MMC, Resnest_MMC, Seresnext_MMC, Effnet_MMC_Multitask
import cv2
...
def val_epoch(model, loader, target_idx, is_ext=None, n_test=1, get_output=False):
    ...
    with torch.no_grad():
        if args.side_task_type:
            for (data, target, target2, patients_id, image_name) in tqdm(loader):
                ...
                if args.use_meta:
                    ...
                    data, target, target2, patients_id = data.to(device), target.to(device), target2.to(device), patients_id.to(device)
                    logits = torch.zeros((data.shape[0], args.out_dim)).to(device)
                ...
                PATIENT_ID.append(patients_id.detach().cpu())
            ...
            for (data, target, patient_id, image_name) in tqdm(loader):
                if args.use_meta:
                    ...
                    data, target, patient_id = data.to(device), target.to(device), patient_id.to(device)
                    logits = torch.zeros((data.shape[0], args.out_dim)).to(device)
                    probs = torch.zeros((data.shape[0], args.out_dim)).to(device)
                    for I in range(n_test):
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, target_idx):
    ...
    # 데이터셋 읽어오기
    dataset_train = MMC_ClassificationDataset(df_train, 'train', meta_features, transform=transforms_train, side_task=args.side_task_type)
    dataset_valid = MMC_ClassificationDataset(df_valid, 'valid', meta_features, transform=transforms_val, side_task=args.side_task_type)
...
if __name__ == '__main__':
    ...
    print('**** dataset.py train.csv --> train_half.csv로 변경 ****')
    print('----------------------------')
